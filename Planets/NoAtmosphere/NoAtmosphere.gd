...
func set_pixels(amount):
	$Ground.material.set_shader_param("pixels", amount)
	$Craters.material.set_shader_param("pixels", amount)

	$Ground.rect_size = Vector2(amount, amount)
	$Craters.rect_size = Vector2(amount, amount)

func set_light(pos):
	$Ground.material.set_shader_param("light_origin", pos)
	$Craters.material.set_shader_param("light_origin", pos)

func set_seed(sd):
	var converted_seed = sd%1000/100.0
	$Ground.material.set_shader_param("seed", converted_seed)
	$Craters.material.set_shader_param("seed", converted_seed)

func set_rotate(r):
	$Ground.material.set_shader_param("rotation", r)
	$Craters.material.set_shader_param("rotation", r)

func update_time(t):
	$Ground.material.set_shader_param("time", t * get_multiplier($Ground.material) * 0.02)
	$Craters.material.set_shader_param("time", t * get_multiplier($Craters.material) * 0.02)

func set_custom_time(t):
	$Ground.material.set_shader_param("time", t * get_multiplier($Ground.material))
	$Craters.material.set_shader_param("time", t * get_multiplier($Craters.material))

func set_dither(d):
	$Ground.material.set_shader_param("should_dither", d)

func get_dither():
	return $Ground.material.get_shader_param("should_dither")
...
func set_colors(colors):
	_set_colors_from_vars($Ground.material, color_vars1, colors.slice(0, 2, 1))
